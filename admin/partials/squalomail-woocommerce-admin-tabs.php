    ...
    <p class="sqm-mc-woocommerce-settings-subtitles">
        ...
        if ($active_tab == 'sync' && $show_sync_tab) {
            if (squalomail_is_done_syncing()) {
                ...
                wp_kses(_e('Connect your WooCommerce store to a<br/>Mailchimp audience in less than 60 seconds', 'squalomail-for-woocommerce'), $allowed_html);
            }
        }
 
        if ($active_tab == 'logs' && $show_sync_tab) {
            wp_kses(_e('Log events from the <br/>Mailchimp plugin', 'squalomail-for-woocommerce'), $allowed_html);
        }
        ?>
    </p>
    <?php if($show_wizard): ?>  
